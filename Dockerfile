...
ENV NLTK_DATA /backend/nltk_data
RUN python -m nltk.downloader -d /backend/nltk_data averaged_perceptron_tagger
RUN python -m nltk.downloader -d /backend/nltk_data cmudict
...
# Copy the entire backend directory and app.py into the container
COPY . /backend
